...
const setupStrapi = async () => {
  if (!instance) {
    /** the following code in copied from
     * `./node_modules/strapi/lib/Strapi.js` */
...
/**
 * Grants database `permissions` table that role can access
 * an endpoint/controllers
 *
 * @param {int} roleID, 1 Autentihected, 2 Public, etc
 * @param {string} value, in form or dot string eg
 * `"permissions.users-permissions.controllers.auth.changepassword"`
